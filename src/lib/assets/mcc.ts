...
export const lookup = (mcc: number) => {
	const result = mccLookup.find((entry) => entry.mcc === mcc);
	if (result === undefined || result === null) return null;

	const country = countries.find((country) => country.iso === result.iso);
